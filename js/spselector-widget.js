cpdefine("inline:com-chilipeppr-widget-serialport", ["chilipeppr_ready", "jquerycookie"], function () {
	return {
		...
		name: "Serial Port Selector",
		desc: "This widget shows your available serial ports. It must connect with your Serial Port JSON server that ChiliPeppr provides for Windows, Mac, and Linux.",
		url: "http://fiddle.jshell.net/baimar97/cojfazjq/show/light/",       // The final URL of the working widget as a single HTML file with CSS and Javascript inlined. You can let runme.js auto fill this if you are using Cloud9.
		fiddleurl: "http://jsfiddle.net/baimar97/cojfazjq/", // The edit URL. This can be auto-filled by runme.js in Cloud9 if you'd like, or just define it on your own to help people know where they can edit/fork your widget
...
`,
		...
		foreignPublish: {
			...
			'/com-chilipeppr-interface-cnccontroller/plannerpause' : 'We publish a planner pause if we see that the buffer count in the Serial Port JSON Server gets above 2000.',
			'/com-chilipeppr-interface-cnccontroller/plannerresume' : 'We publish a planner resume when we get back to 1000.'
		},
		isWsConnected: false,
		host: null,
		portlist: null,
		...
		init: function (host, buffertype, defaultBaud, buffertypeDescription) {
			...
			// setup recv pubsub event com-chilipeppr-widget-serialport
			...
			chilipeppr.subscribe("/com-chilipeppr-interface-cnccontroller/portinitialized", this, function () {
				console.log("sendStartScript(this,this.portlist[0])")
				//this.sendStartupScript(this,this.portlist[0]);
			});
		...
		btnBarSetup: function () {
            ...
            */
			...
			$('.com-chilipeppr-widget-serialport .btn.spjs-restart').click(function () {
				that.restartSpjs(that);
			});

			this.forkSetup();

		},
		restartSpjs: function(that) {
			console.log("restartSpjs()");
			this.wsSend("restart");
			setTimeout(function() {
				console.log("attempting to reconnect with SPJS");
				that.getPortList();
			}, 4000);
		},
		exitSpjs: function() {
		...
		publishSysMsg: function (msg) {
			...
			if (this.lastMsg == msg && now - this.lastMsgTime < 20000) {
				// skip publish
				console.log("skipping publish. same msg and too fast.");
			} else {
		...
		onPortOpen: function(data) {
			...
			console.log("the script key:", key);
		...
		sendStartupScript: function(that,item) {
			console.group("Sending Startup Scripts");
			console.log("that.host: ", that.host);
			console.log("that.portlist: ", that.portlist);	//portlist stores the port data of all connections available on the SPJS
			console.log("item: ", item);	//item stores the port data of the currently open port

			/*for (var openPort = 0; (that.portlist[openPort] != undefined) && (!that.portlist[openPort]['IsOpen']); openPort++) {
				console.log("openPort: ",openPort);
			}*/

			if (that.host == "cnc-machine") {
				console.log("running PunchPress startup scripts")
				var initCmds = [
					{ cmd: 'G17', pauseAfter: 50 }, //xy work plane
					{ cmd: 'G94', pauseAfter: 50 }, //units per minute feedrate mode
					{ cmd: 'G90', pauseAfter: 50 }, //absolute distance mode
					{ cmd: 'G21', pauseAfter: 200 }, //millimeters mode
					'(Writing Settings To Control Board)',
					{ cmd: '{"z":{"am":1,"vm":17000,"fr":17000,"tn":0,"tm":2440,"jm":300,"jh":800,"jd":0.05,"sn":1,"sx":0,"sv":2000,"lv":400,"lb":25,"zb":0.254}}', pauseAfter: 200},
					{ cmd: '{"y":{"am":1,"vm":3800,"fr":3800,"tn":0,"tm":720,"jm":15,"jh":100,"jd":0.05,"sn":1,"sx":0,"sv":1000,"lv":200,"lb":100,"zb":6.35}}', pauseAfter: 200},
					{ cmd: 'G20', pauseAfter: 200 }, //inches mode
					{ cmd: '{"2":{"ma":2,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3}}', pauseAfter: 200},
					{ cmd: '{"3":{"ma":1,"sa":1.8,"tr":1.1515,"mi":8,"po":1,"pm":3}}', pauseAfter: 200},
					'(Calibrating Punch Press Gates)',
					{ cmd: 'M08', pauseAfter: 250}, //lift finger solenoid
					{ cmd: 'G28.2 Y0 Z0', pauseAfter: 1000 }, //home axes
					{ cmd: 'M09', pauseAfter: 250 }, //drop finger solenoid
					{ cmd: 'G10 L2 P1 Y-3.614 Z-6.5', pauseAfter: 200 }, //set g54 work offsets
					'(Connected to Punch Press Controller)'
				];
			} else if (that.host == "Braydens-Laptop") {
				console.log("running startup scripts for Braydens-Laptop");
				console.log("item.SerialNumber:",item.SerialNumber);
				//if (item.SerialNumber == "FTDIBUS\VID_0403+PID_6015+DA01138SA\0000") {
				if (item.DisplayName == "USB Serial Port (COM3)") {
					console.log("TinyG board detected");
					var initCmds = [
						'(Init Scripts for TinyG - Sending)',
						{ cmd: 'M08', pauseAfter: 150 },
						'(Init Scripts for TinyG - Completed)'
					];
				} else if (item.DisplayName == "TinyG G2") {
					console.log("TinyG G2 board detected");
					var initCmds = [
						'(TinyG G2 Board Detected)'
						/*{ cmd: '{"x":{"am":1,"vm":1000,"fr":1000,"tn":0,"tm":1000,"jm":20,"jh":40,"jd":0.05,"sn":1,"sx":0,"sv":500,"lv":100,"lb":5,"zb":1}}', pauseAfter: 300 },
						{ cmd: '{"y":{"am":1,"vm":1000,"fr":1000,"tn":0,"tm":1000,"jm":20,"jh":40,"jd":0.05,"sn":1,"sx":0,"sv":500,"lv":100,"lb":5,"zb":1}}', pauseAfter: 300 },
						{ cmd: '{"z":{"am":1,"vm":1000,"fr":1000,"tn":0,"tm":1000,"jm":20,"jh":40,"jd":0.05,"sn":1,"sx":0,"sv":500,"lv":100,"lb":5,"zb":1}}', pauseAfter: 300 },
						{ cmd: '{"a":{"am":3,"sn":1,"vm":230400,"sx":0,"fr":230400,"sv":600,"tm":-1,"lv":100,"tn":-1,"lb":5,"jd":0.05,"zb":2,"jm":5760,"jh":11520,"ra":0.008}}', pauseAfter: 300},
						{ cmd: '{"b":{"am":3,"sn":1,"vm":230400,"sx":0,"fr":230400,"sv":600,"tm":-1,"lv":100,"tn":-1,"lb":5,"jd":0.05,"zb":2,"jm":5760,"jh":11520,"ra":0.008}}', pauseAfter: 300},
						{ cmd: '{"c":{"am":3,"sn":1,"vm":230400,"sx":0,"fr":230400,"sv":600,"tm":-1,"lv":100,"tn":-1,"lb":5,"jd":0.05,"zb":2,"jm":5760,"jh":11520,"ra":0.008}}', pauseAfter: 300},
						{ cmd: '{"1":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}', pauseAfter: 300},
						{ cmd: '{"2":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}', pauseAfter: 300},
						{ cmd: '{"3":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}', pauseAfter: 300},
						{ cmd: '{"4":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}', pauseAfter: 300},
						{ cmd: '{"5":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}', pauseAfter: 300},
						{ cmd: '{"6":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}', pauseAfter: 300}*/
					];
				} else {
					console.log("board not recognized");
					var initCmds = [
						'(GG - Board Not Recognized - git ouuta here >:U)'
					];
				}
			} else {
				console.log("board not recognized. no startup scripts to run");
				var initCmds = [
					'(no startup scripts to send  ~(*.*)~)'
				];
			}
			console.log("startup scripts:", initCmds);
			...
			if (initCmds) {
				setTimeout(function () {
					for (var cmdCtr = 0; cmdCtr < initCmds.length; cmdCtr++) {
						...
						console.log("rawCmd.slice(0,1) + rawCmd.slice(-1): ", rawCmd.slice(0,1) + rawCmd.slice(-1));
						if ((rawCmd.slice(0,1) + rawCmd.slice(-1)) == "()") {
							console.log("publishSysMsg(rawCmd)");
							that.publishSysMsg(rawCmd);
						} else {
							rawCmd += "\n";
				...
				console.log("startup scripts are undefined. not sending startup scripts");
			}
			/* JSON Settings Script for Thermika CNC Router
			{"x":{"am":1,"sn":0,"vm":984,"sx":0,"fr":394,"sv":20,"tm":29.724,"lv":4,"tn":0,"lb":0.197,"jd":0.002,"zb":0.039,"jm":35,"jh":2}}
			{"y":{"am":1,"sn":0,"vm":1181,"sx":0,"fr":394,"sv":20,"tm":61.299,"lv":4,"tn":0,"lb":0.197,"jd":0.002,"zb":0.039,"jm":1,"jh":2}}
			{"z":{"am":1,"sn":0,"vm":31,"sx":0,"fr":31,"sv":16,"tm":2.953,"lv":4,"tn":0,"lb":0.197,"jd":0.002,"zb":0.039,"jm":1,"jh":2}}
			{"a":{"am":0,"sn":1,"vm":230400,"sx":0,"fr":230400,"sv":600,"tm":-1,"lv":100,"tn":-1,"lb":5,"jd":0.05,"zb":2,"jm":5760,"jh":11520,"ra":0.008}}
			{"1":{"ma":0,"pm":1,"po":0,"tr":1.5701,"mi":8,"sa":1.8}}
			{"2":{"ma":1,"pm":1,"po":0,"tr":1.5701,"mi":8,"sa":1.8}}
			{"3":{"ma":1,"pm":1,"po":1,"tr":1.5701,"mi":8,"sa":1.8}}
			{"4":{"ma":2,"pm":3,"po":1,"tr":0.0492,"mi":2,"sa":1.8}}
			*/

			/* JSON Test Settings Script for TinyG G2 Board
			{"x":{"am":1,"vm":1000,"fr":1000,"tn":0,"tm":1000,"jm":20,"jh":40,"jd":0.05,"sn":1,"sx":0,"sv":500,"lv":100,"lb":5,"zb":1}}
			{"y":{"am":1,"vm":1000,"fr":1000,"tn":0,"tm":1000,"jm":20,"jh":40,"jd":0.05,"sn":1,"sx":0,"sv":500,"lv":100,"lb":5,"zb":1}}
			{"z":{"am":1,"vm":1000,"fr":1000,"tn":0,"tm":1000,"jm":20,"jh":40,"jd":0.05,"sn":1,"sx":0,"sv":500,"lv":100,"lb":5,"zb":1}}
			{"a":{"am":3,"sn":1,"vm":230400,"sx":0,"fr":230400,"sv":600,"tm":-1,"lv":100,"tn":-1,"lb":5,"jd":0.05,"zb":2,"jm":5760,"jh":11520,"ra":0.008}}
			{"b":{"am":3,"sn":1,"vm":230400,"sx":0,"fr":230400,"sv":600,"tm":-1,"lv":100,"tn":-1,"lb":5,"jd":0.05,"zb":2,"jm":5760,"jh":11520,"ra":0.008}}
			{"c":{"am":3,"sn":1,"vm":230400,"sx":0,"fr":230400,"sv":600,"tm":-1,"lv":100,"tn":-1,"lb":5,"jd":0.05,"zb":2,"jm":5760,"jh":11520,"ra":0.008}}
			{"1":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}
			{"2":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}
			{"3":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}
			{"4":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}
			{"5":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}
			{"6":{"ma":0,"sa":1.8,"tr"0.5236:,"mi":8,"po":1,"pm":3,"pl":0.375}}
			*/
		...
		sendPauseAt: 2000, // we will ask the Gcode widget to pause send if our buffer gets this high
		sendResumeAt: 1000, // we will send resume when buffer gets back here
		...
		deviceMeta: {
			...
			"arduino_due_x_dbg" : {
				...
				buffer: "default",
				baud: "9600",
				img: "icons/duetrans100wide150tall.png"
			},
			"uno" : {
		...
		onPortList: function (portlist) {
			...
			$.each(portlist, function (i, item) {
				...
				if (item.IsOpen) {
					...
					if (that.isInitting) {
						...
						console.log("onPortList requesting startup scripts be sent");
						that.sendStartupScript(that,item);
					}
				}
			...
			for (var count = 0; portlist[count] != undefined; count++) {
				console.log("count: ",count);
				if (portlist[count]['metaBuffer'] == "tinyg" || portlist[count]['metaBuffer'] == "tinygg2") {
					console.log("portlist[count]['metaBuffer']: ", portlist[count]['metaBuffer']);
					this.onPortFriendlyClicked({data:{that:this, port:portlist[count]}});
				}
			}
		...
		onPortCbClicked: function(evt) {
			//TODO: make it such hat when you are connected to a board and the page is refreshed, the refreshed page connects to the previously connected board rather than disconnecting from it.
			console.group("onPortCbClicked");
			console.log("evt: ", evt);
			var item = evt.data.port;
			var that = evt.data.that;
			console.log("item: ",item);
			...
			if (isSelected) {
				...
				that.publishSysMsg("Serial port " + item.Friendly + " opened with baud rate " + baud);
				...
				that.publishSysMsg("Serial port " + item.Friendly + " closed");
			}
			console.groupEnd();
		},
		...
		onWsConnect: function (event) {
			...
			$('#com-chilipeppr-widget-serialport-portstab').tab('show');
